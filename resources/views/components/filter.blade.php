<form method="GET" action="{{ route('apartment.all') }}">
    <div class="flex items-center justify-center p-4">
        <button id="filterDropdownBtn" data-dropdown-toggle="filterDropdown"
            class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:ring-2 focus:ring-indigo-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 transition"
            type="button">
            <span>Filter By</span>
            ...
            <ul class="p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200">
                <li>
                    <label class="flex items-center cursor-pointer group">
                        <input onchange="this.form.submit()" @checked(in_array('apartments', request('filter', []))) 
                            name="filter[]" type="checkbox" value="apartments"
                ...
                <li>
                    <label class="flex items-center cursor-pointer group">
                        <input onchange="this.form.submit()" @checked(in_array('bookings', request('filter', []))) 
                            name="filter[]" type="checkbox" value="bookings"
                            class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer">
