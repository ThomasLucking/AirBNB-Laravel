    ...
    <div class="flex items-center justify-center p-4">
        ...
        <div id="sortingDropdown"
            ...
            <ul class="p-3 space-y-1">
                <li>
                    <label
                        class="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer group">
                        <input onchange="this.form.submit()" @checked(request()->filled('sort_price')) id="sort_price"
                            type="checkbox" name="sort_price" value="sort_price"
                            class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-500" />
                        <span
                            class="text-sm text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white">Price</span>
                    </label>
                </li>
                <li>
                    <label
                        class="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer group">
                        <input onchange="this.form.submit()" @checked(request()->filled('sort_rooms')) id="sort_rooms"
                            type="checkbox" name="sort_rooms" value="sort_rooms"
                            class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-500" />
                        <span
                            class="text-sm text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white">Number
                            of Rooms</span>
                    </label>
                </li>
            </ul>
            ...
            @if(request()->filled('sort_price') || request()->filled('sort_rooms'))
                <div
                    class="py-2 px-3 bg-gray-50 dark:bg-gray-800 rounded-b-xl border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('apartment.all', array_filter(request()->only(['apartments', 'bookings']))) }}">"
